...
# Set sizes
Size_key = 32
SizeIv = AES.block_size if mode == AES.MODE_CFB else 0
# Loading original image
original = c.imread("true1.jpg")
originalRow, origialCol, originalDepth = original.shape
# Checking for minimum width
min_width = (AES.block_size + AES.block_size) // originalDepth + 1
if origialCol < min_width:
 print('The minimum width of the image must be {} pixels, so that IV and padding can be stored in a single additional row!'.format(min_width))
 s.exit()
# Displaying the original image
c.waitKey()
c.imwrite("toriginalimg.bmp", original)
original = c.imread("toriginalimg.bmp")
# Converting original image data to bytes
#*************************************************
bytesOriginal = original.tobytes()
# Encrypting 
KEY = b'\xab\x91\n$\x11\xdc5\tZaG\xf4T$"N\x8aHe+\x96\xe6\xfd\xfa\xaa\xc3\xbccx!=*'
IV = b'\xf9\x96\x9f\xc0\x00\xd9\xd3lYT\x1d;3-\xa1%'
paddedBytesOriginal = pad(bytesOriginal, AES.block_size)
SizePadded = len(paddedBytesOriginal) - len(bytesOriginal)
zeros = origialCol * originalDepth - SizeIv - SizePadded
if mode == AES.MODE_CFB:
 cipher = AES.new(KEY, AES.MODE_CFB, IV)
 cipherImgBytes = cipher.encrypt(paddedBytesOriginal)
 ivCiphertextVoid = IV + cipherImgBytes + bytes(zeros)
 encryp = np.frombuffer(ivCiphertextVoid, dtype = original.dtype).reshape(originalRow + 1, origialCol, 
originalDepth)
 c.imshow("Encrypted image", encryp)
 c.imwrite("Encrypted.bmp", encryp)
 encryp = c.imread("Encrypted.bmp")
# Display encrypted image
c.imshow("Encrypted image", encryp)
c.waitKey()
######################################################################################
myImage = Image.open("Encrypted.bmp");
...
new_image.save('C:\\Users\\hp\\OneDrive\\Desktop\\mini project\\demo\\aes_encrypted.jpg')
